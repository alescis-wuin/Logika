...
final class InteractionFeedbackResolver {
    WireTargetFeedback resolveWireTarget(Camera2D camera, Viewport viewport, Circuit circuit, PinRef pendingWire,
        ...
        if (pendingWire == null || !validPin(circuit, pendingWire)) return WireTargetFeedback.none();
        if (hoveredPin != null && !hoveredPin.equals(pendingWire)) return new WireTargetFeedback(hoveredPin, canConnect(circuit, pendingWire, hoveredPin));
        Vec2 mouseWorld = camera.screenToWorld(new Vec2(mouseX, mouseY), viewport);
        double radius = UiMetrics.PIN_HIT_RADIUS_SCREEN / Math.max(0.18, camera.zoom());
        PinRef bestRef = null;
        double bestDistance = Double.POSITIVE_INFINITY;
        for (CircuitComponent component : circuit.components()) {
            for (PinEndpoint pin : component.pins()) {
                PinRef candidate = pin.ref();
                if (candidate.equals(pendingWire) || !canConnect(circuit, pendingWire, candidate)) continue;
    ...
    CursorFeedback.Style resolveCursor(Toolbar toolbar, Viewport viewport, List<PlacementPreview> placementPreviews,
                                       ...
                                       boolean draggingComponent, PinRef hoveredPin, WireId hoveredWireId,
                                       int hoveredWireControlPointIndex, boolean draggingWireControlPoint,
                                       WireTargetFeedback targetFeedback, double mouseX, double mouseY) {
        if (draggingWireControlPoint || hoveredWireControlPointIndex >= 0) return CursorFeedback.Style.MOVE;
        if (hoveredPin != null || targetFeedback.active()) return CursorFeedback.Style.WIRE;
        if (draggingComponent || (hoveredComponentId != -1 && selectedComponentIds.contains(hoveredComponentId))) return CursorFeedback.Style.MOVE;
        if (toolbar.actionAt(mouseX, mouseY, viewport.windowWidth(), viewport.windowHeight()).isPresent()) return CursorFeedback.Style.HAND;
        if (hoveredPlacementPreview(placementPreviews) || hoveredComponentId != -1 || hoveredWireId != null) return CursorFeedback.Style.HAND;
        return CursorFeedback.Style.DEFAULT;
    }

    private boolean hoveredPlacementPreview(List<PlacementPreview> placementPreviews) {
        for (PlacementPreview preview : placementPreviews) if (preview.hovered()) return true;
        return false;
    }

    private boolean canConnect(Circuit circuit, PinRef a, PinRef b) {
        if (a.equals(b) || a.isOutput() == b.isOutput()) return false;
        PinRef output = a.isOutput() ? a : b;
        PinRef input = a.isInput() ? a : b;
        if (output.componentId() == input.componentId()) return false;
        if (!validPin(circuit, output) || !validPin(circuit, input)) return false;
        return !circuit.hasWire(output, input);
    }
